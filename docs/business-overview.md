...
The `evaluation` domain exists: plain TypeScript types for a scenario, an individual check, and the pass/fail result computed from those checks. `specs/scenario-spec.md` is the actual spec — the types and tests are derived from it, not the reverse. There's no way yet to load a scenario from a file, run one against a real agent, call a model, retrieve documents, or expose any of this over an API or dashboard.
